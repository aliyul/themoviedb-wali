...
@section('content')
    ...
    <div class="movie-info border-b border-gray-800">
        <div class="container mx-auto px-4 py-16 flex flex-col md:flex-row">
            <form class="card" role="form" action="{{ route('movies.store') }}" method="POST" >
                @csrf
                @method('POST')
                <div class="flex-none">
            ...
            <div class="md:ml-24">
                ...
                <div class="flex flex-wrap items-center text-gray-400 text-sm">
                    ...
                    <input type="hidden" name="vote_average" class="form-control" value="{{ $movie['vote_average'] }}">
                    <span class="mx-2">|</span>
                    <span>{{ $movie['release_date'] }}</span>
                    <input type="hidden" name="release_date" class="form-control" value="{{ $movie['release_date'] }}">
                    <span class="mx-2">|</span>
                    <span>{{ $movie['genres'] }}</span>
                    <input type="hidden" name="genres" class="form-control" value="{{ $movie['genres'] }}">
                </div>

                <p class="text-gray-300 mt-8">
                    {{ $movie['overview'] }}
                </p>
                <input type="hidden" name="overview" class="form-control" value="{{ $movie['overview'] }}">
                ...
                <div x-data="{ isOpen: false }">
                    @if (count($movie['videos']['results']) > 0)
                        <div class="mt-6">
                            <button @click="isOpen = true" class=" col-md-6 flex inline-flex items-center bg-orange-500 text-gray-900 rounded font-semibold px-5 py-4 hover:bg-orange-600 transition ease-in-out duration-150">
